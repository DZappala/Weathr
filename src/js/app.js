...
$(document).ready(function () {
    ...
    $('#weatherSearch').click((event) => {
        ...
        fetch('https://maps.googleapis.com/maps/api/place/autocomplete/json?input=' + value + '&types=(cities)&key=' + googleMapsKey, {
            mode: 'no-cors'
        }).then(function (response) {
            console.log("response", response);
            return response;
            ...
            console.log("response", response);
            let query = response.predictions[0].terms[0].value + ',' + response.predictions[0].terms[(terms.length)].value;

            // fetch data from OpenWeatherMap API 
            fetch("https://api.openweathermap.org/data/2.5/weather?q=" + query + "&type=like&units=imperial&APPID=" + apiKey).then(function (response) {
                console.log("response", response);
                return response.json();
            }).then(function (response) {
                console.log("response", response);
